async def bot_command(self, message, MEMBER_ROLE_ID, OFFICER_ROLE_ID, PROJECT_ROLE_ID, RECRUIT_ROLE_ID):
    """
    ...
    try:
        if '/CreateProject' in message.content:
            ...
            if OFFICER_ROLE_ID in list(map(lambda role: role.id, message.author.roles)):
                author_dm = await message.author.create_dm()
                # Attempt to split and save the project name
                try:
                    if len(message.content.split(' '))<2:
                        await author_dm.send("Project name is empty")
                    ...
                    if projectName in [channel.name for channel in category.channels]:
                        async with author_dm.typing():
                            await author_dm.send(f"The project, {projectName}, already exists!")
                    elif projectName in [role.name for role in message.guild.roles]:
                        async with author_dm.typing():
                            await author_dm.send(f"The role, {projectName}, already exists!")
                    else:
            ...
            if OFFICER_ROLE_ID in list(map(lambda role: role.id, message.author.roles)):
                ...
                try:
                    ...
                    if projectName in [channel.name for channel in category.channels]:
                        ...
                        async with author_dm.typing():
                            await author_dm.send(f"The project, {projectName}, doesn't exist!")
                
                except Exception as e:
                    print(f"User entry failed: {message.content} \n {e}")
                    async with author_dm.typing():
                        await author_dm.send("***Error deleting the project...***\nPlease use the format: `/DeleteProject projectName` \n Where ProjectName is the name of the project")
    except Exception as e:
